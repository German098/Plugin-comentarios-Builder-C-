...
void __fastcall AddSplashScreen( )
{
	...
	if ( SplashScreenServices->Supports( SSServices ) ) {
		...
		SSServices->AddPluginBitmap( Format( strSplashScreenName, ARRAYOFCONST( ( iMajor, iMinor, strRev, Application->Title ) ) ), bmSplashScreen, False, Format( strSplashScreenBuild, ARRAYOFCONST( ( iMajor, iMinor, iBugFix, iBuild ) ) ) );
